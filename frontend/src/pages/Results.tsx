...
const Results = ({name, ingredient}) => {
  let {data} = useQuery(["recipe"], async () => {
  ...
  return (
    <div className='parent space-y-4 pt-1'>
      {data?.data.map((recipe: any)=>    
          <div className="child max-w-sm p-6 bg-inherit backdrop-blur-xl font-mono border border-stone-800 rounded-lg shadow text-zinc-800 dark:border-gray-700">
